...
int main(int argc, char **argv)
{
    ...
    int         c,              /* option index                              */
                ...
                pert,
#if defined(START_SIGNATURE)
                start= START_SIGNATURE, /* Starting value for signature curve*/
#else
...
#define TLAST 100
  ...
  for (i=TLAST;(i<TLAST*10e8)&&((t1-t0) < 0.000001);i*=10)
   {
   t0 = When();
      for( n=0; n< i ; n++) {
         if( args.tr) {
   ...
   tlast = (t1 - t0) / i * 2;
#if 0
   fprintf(stderr,"i: %d, t0: %lf, t1: %lf, t1-t0: %lf, tlast: %f\n",
           i,t0,t1,t1-t0,tlast);
#endif
   }
...
/*                    SendTime(&args, &t2);*/
            ...
            if ( args.tr ) {
               if(integCheck) {
                 ...
                 fprintf(stderr," %10.2lf Mbps in %10.2lf usec\n", 
...
#if defined(STRLEN)
  /* If using strlen, then we need to zero terminate receive string.
   */   
    p->r_buff[(nbytes+MAX(soffset,roffset))-1] = '\0';
#endif
